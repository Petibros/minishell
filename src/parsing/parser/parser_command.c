...
static t_token	*create_word_token(t_token *src)
{
	t_token	*new_word;
...
static int	process_command_token(t_nodes *node, t_token **token,
...
t_nodes	*parse_command(t_token **token)
{
	...
	{
		if (!process_command_token(node, token, &word_tokens, &word_count))
			return (NULL);
	}
