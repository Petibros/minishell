...
/*   Updated: 2025/03/15 23:29:12 by sacgarci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_path(char *cmd, char **envp)
{
	int		i;
	char	**paths;
	char	*path;
...
void	exec_cmd(t_vars *vars, t_nodes **cmds, int pipes[2][2])
{
	...
	if (vars->cmd.fd_in == -1 || vars->cmd.fd_out == -1)
		exit_error(1);//fonction qui free tout les pointeurs du processus fils
	dup2(vars->cmd.fd_in, 0);
	dup2(vars->cmd.fd_out, 1);
	close_fds();//fonction qui close tous les fds ouverts du processus fils
	...
	execve(path, *cmds->argv, vars->env.envp);
	if (path && path[0] == '.' && path[1] == '/')//si c'est un executable
		path = &path[2];
	if (!path || access(path, F_OK) != 0)
		exit_error(127);//command not found ou path pas trouve dans l'environnement
	else if (access(path, X_OK) != 0)
		exit_error(126);//not enough privileges
	else
		exit_error(2);//invalid option(2)
}
